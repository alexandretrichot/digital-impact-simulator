...
export const GroupPage: React.FC = () => {
  ...
  return (
    <div id='group' className='page'>
      {groupSlug ? <SimulateWithGroupView groupSlug={groupSlug} /> : <CreateGroupView />}
    </div>
  );
};
      ...
      <Formik
        ...
        onSubmit={(values, helpers) => {
          fetcher<Group>('/api/groups', 'POST', {
            ...
            .then((r) => navigate(`/session/${r.slug}/dashboard`))
        ...
        {({ values, errors, touched, handleSubmit, handleBlur, handleChange, isSubmitting }) => {
          ...
          return (
            <form onSubmit={handleSubmit} style={{ maxWidth: '20rem', margin: '2rem auto' }}>
              ...
              <p style={{ backgroundColor: '#eee', borderRadius: '.2rem', padding: '.1rem .2rem', overflowX: 'scroll' }}>
                numerique.meusenature.fr/session/{slug ? <span>{slug}</span> : <span style={{ opacity: 0.3 }}>{'<nom>'}</span>}
              </p>
              <button type='submit' className='btn large' disabled={isSubmitting}>
                Créer le groupe
              </button>
            </form>
          );
        ...
        <>
          ...
          <div className='wrapper'>
            <header>
              ...
              <p>
                Vous êtes dans une simulation de groupe. Cela signifie que les résultats de votre simulation seront visibles en temps réel sur{' '}
                <a href={`/group/${group?.slug}/dashboard`} target='_blank' rel='noreferrer'>
                  cette page
                </a>
                .
              </p>
              ...
              <div className='user'>
                <p>
                  Votre nom dans ce groupe :{' '}
                  <input
                    value={member.name}
                    placeholder='Sans nom (cliquez pour éditer)'
                    onChange={(ev) => setMember({ ...member, name: ev.currentTarget.value })}
                  />
                </p>
              </div>
            </header>

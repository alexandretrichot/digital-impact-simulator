					...
					error ? (
						...
						group && (
							<>
								...
								<DashboardPanel groupSlug={groupSlug} groupName={group.name} members={sortedMembers} />
							</>
						)
					)
				)
		...
		<>
			<div className="wrapper">
				<header>
					...
					<div className="link">Lien du groupe : <code>{joinLink}</code></div>
				</header>
			</div>
						...
						{props.members.map((m, index) => (
							<div key={m.id} className="member">
								<div>
									...
									<div className="" style={{ width: gesTotals[index] / maxGES * 100 + '%' }}><Graph stats={getGesStats(m.stats)} /></div>
								</div>
